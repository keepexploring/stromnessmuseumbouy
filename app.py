...
deployment_date_str = os.getenv("DEPLOYMENT_DATE", "2025-06-30")
try:
    DEPLOYMENT_DATE = datetime.strptime(deployment_date_str, "%Y-%m-%d")
except ValueError:
    DEPLOYMENT_DATE = datetime(2025, 6, 30)  # Fallback to project start
...
def load_temperature_data(hours_back):
    try:
        # Calculate start time, but enforce minimum cutoff date
        ...
        # Query Supabase with date restriction
...
if st.session_state.get('show_date_picker', False):
    ...
    if st.button("Download Custom Range"):
        # Enforce data cutoff for custom downloads too
